...
int handleSenderResponse(int clientSocket) {
    ...
    if (strcmp(response, "n") == 0) {
        printf("Received termination signal from sender. Closing connection.\n");
        return 0; // Exit the loop
    } else if (strcmp(response, "y") == 0) {
        printf("Received resend signal from sender. Receiving file again.\n");
...
int main(int argc, char *argv[]) {
    ...
    printf("- * Statistics * -\n");
    for (int i = 0; i < totalFilesReceived; i++) {
        printf("Run #%d Data: Time=%.2fms; Speed=%.2fKB/s\n", i + 1, runTime[i], runSpeed[i]);
    }
